  ...
  return (
    <AppLayout>
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              {note.isChildSafe && (
                <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded-full">Child safe</span>
              )}
            </div>
            <h1 className={`font-bold text-gray-900 ${isShoppingList ? 'text-2xl' : 'text-xl'}`}>
              {note.title}
            </h1>
          </div>
          {!user?.isChild && (
        ...
        {/* Checklist */}
        <section>
          <h2 className="text-sm font-semibold text-gray-500 mb-2">Checklist</h2>
          ...
          <ul className="space-y-1">
            {note.checklistItems.map(item => (
              <li key={item.id} className="flex items-center gap-3 group">
                ...
                />
                <span className={`flex-1 ${isShoppingList ? 'text-lg' : 'text-sm'} ${item.isChecked ? 'line-through text-gray-400' : 'text-gray-700'}`}>
                  {item.text}
                </span>
                {!user?.isChild && (
