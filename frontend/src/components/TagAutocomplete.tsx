      ...
      {!readOnly && (
        <div ref={wrapperRef} className="relative">
          <input
            ...
            placeholder="Add tag…"
            className="w-48 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 px-3 py-1.5 text-xs focus:outline-none focus:ring-2 focus:ring-primary-500"
          />
          {open && (
            <ul className="absolute z-20 mt-1 w-56 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-lg max-h-48 overflow-y-auto">
              {filtered.map(tag => (
                <li key={tag.id}>
                  <button
                    type="button"
              ...
              {input.trim() && !suggestions.some(s => s.name.toLowerCase() === input.trim().toLowerCase()) && (
                <li>
                  <button
                    ...
                    className="w-full text-left px-3 py-2 text-xs text-primary-600 dark:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/30"
                  >
                    + Create "{input.trim()}"
                  </button>
                </li>
              ...
              {filtered.length === 0 && !input.trim() && (
                <li className="px-3 py-2 text-xs text-gray-400 dark:text-gray-500">No tags yet</li>
              )}
            </ul>
          )}
        </div>
